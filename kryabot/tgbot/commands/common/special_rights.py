...
async def add_special_right(right_type, cmd):
    user_db = await cmd.event.client.db.getUserByTgChatId(cmd.reply_message.sender_id)
    if user_db is None or len(user_db) == 0:
        await cmd.reply_fail(cmd.get_translation('CMD_ADDSPECIAL_NOT_VERIFIED'))
        return

    if await cmd.is_chatsudo(user_db[0]['user_id'], cmd.reply_message.sender_id):
        await cmd.reply_fail(cmd.get_translation('CMD_ADDSPECIAL_IS_SUDO'))
        return

    user = await cmd.client.get_entity(PeerUser(cmd.reply_message.sender_id))
    admin_user = await cmd.client.get_entity(PeerUser(cmd.event.message.sender_id))
    ...
    if right_type == 'WL':
        ...
        await cmd.db.addUserToBlacklist(cmd.channel['channel_id'],
                                          ...
                                          cmd.event.message.sender_id,
                                          text)
        await cmd.client.kick_user_from_channel(cmd.event.message.to_id, cmd.reply_message.sender_id, cmd.channel['ban_time'])
