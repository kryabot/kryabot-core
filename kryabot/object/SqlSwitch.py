...
async def getSql(sqlType):
    return {
        ...
        'find_channel_by_tw_id': 'SELECT * FROM channel LEFT JOIN user on user.tw_id = %s WHERE channel.user_id = user.user_id',
        ...
        'remove_special_right': 'UPDATE tg_special_right SET tg_special_right.deleted = 1, tg_special_right.by_tg_user_id = %s, tg_special_right.comment = %s, tg_special_right.deleted_at = CURRENT_TIMESTAMP WHERE tg_special_right.deleted = 0 and tg_special_right.channel_id = %s and tg_special_right.user_id = %s and tg_special_right.right_type = %s',
        'get_all_tg_special_rights': 'select tg_special_right.*, user.tw_id FROM tg_special_right LEFT JOIN user on user.user_id = tg_special_right.user_id where tg_special_right.deleted = 0;',
        'get_tg_chat_rights': 'select tg_special_right.*, user.tw_id FROM tg_special_right LEFT JOIN user on user.user_id = tg_special_right.user_id where tg_special_right.deleted = 0 and tg_special_right.channel_id = %s',
        ...
        'sp_deleteTgSpecialRight': 'CALL deleteTgSpecialRightByUserId(%s, %s);',
        ...
        'set_subchat_kick_mode': 'UPDATE channel_subchat SET channel_subchat.kick_mode = %s WHERE channel_subchat.tg_chat_id = %s',
        ...
        'get_infobot_links_by_type': 'SELECT * FROM infobot_link WHERE infobot_link.infobot_id = %s AND infobot_link.link_table = %s',
        'get_infobot_twich_links': 'SELECT * FROM infobot_link il LEFT JOIN profile_twitch pt ON pt.profile_twitch_id = il.link_id LEFT JOIN user u ON u.user_id = pt.user_id WHERE il.infobot_id = %s AND il.link_table = "profile_twitch"',
        'save_infobot_config': 'UPDATE infobot_link il SET il.config = %s WHERE il.infobot_id = %s and il.link_table = %s and il.link_id = %s',
        'delete_infobot_link': 'DELETE FROM infobot_link WHERE infobot_link.infobot_id = %s and infobot_link.link_table = %s and infobot_link.link_id = %s;',
